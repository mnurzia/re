...
MN_INTERNAL int mn__str_grow(mn__str* str, mn_size new_size)
{
  ...
  if (MN__STR_GET_SHORT(str)) {
    if (new_size <= MN__STR_SHORT_SIZE_MAX) {
      ...
      /* Needs allocation */
      mn_size new_alloc = MN__STR_ROUND_ALLOC(new_size + (new_size >> 1));
      mn_char* new_data =
    ...
    if (new_size > str->_alloc) {
      ...
      mn_char* new_data;
      if (str->_alloc == 0) {
        new_data = (mn_char*)MN_MALLOC(sizeof(mn_char) * (new_alloc + 1));
      } else {
        new_data =
            (mn_char*)MN_REALLOC(str->_data, sizeof(mn_char) * (new_alloc + 1));
      }
      if (new_data == MN_NULL) {
        return -1;
      }
...
mn_size mn__str_view_size(const mn__str_view* view) { return view->_size; }
