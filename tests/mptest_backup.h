...
#if MPTEST_USE_APARSE
MN_INTERNAL void mptest__aparse_init(struct mptest__state* state);
...
#if MPTEST_USE_SYM
MN_INTERNAL void mptest__sym_dump(mptest_sym* sym, mptest_int32 parent_ref, mptest_int32 indent);
#endif
...
MN_INTERNAL const char* mptest__aparse_help = "Runs tests.";

MN_INTERNAL const char* mptest__aparse_version = MPTEST_VERSION_STRING;
...
MN_INTERNAL int mptest__test_name_match(const char* test_name,
...
MN_INTERNAL mptest__sym_tree* mptest__sym_get(mptest_sym* sym, mptest_int32 ref) {
    MPTEST_ASSERT(ref != MPTEST__SYM_NONE);
    return mptest__sym_tree_vec_getref(&sym->tree_storage, (mptest_size)ref);
}

MN_INTERNAL const mptest__sym_tree* mptest__sym_getcref(const mptest_sym* sym, mptest_int32 ref) {
    MPTEST_ASSERT(ref != MPTEST__SYM_NONE);
    return mptest__sym_tree_vec_getcref(&sym->tree_storage, (mptest_size)ref);
}

MN_INTERNAL int mptest__sym_new(mptest_sym* sym, mptest_int32 parent_ref, mptest_int32 prev_sibling_ref, mptest__sym_tree new_tree, mptest_int32* new_ref) {
    int err = 0;
...
MN_INTERNAL int mptest__sym_isblank(mptest_char ch) {
    return (ch == '\n') || (ch == '\t') || (ch == '\r') || (ch == ' ');
}
...
MN_INTERNAL int mptest__sym_parse_esc(mptest__sym_parse* parse, mptest_char ch) {
    if (ch == 'n') {
        parse->num = '\n';
    } else if (ch == 'r') {
        parse->num = '\r';
...
MN_INTERNAL int mptest__sym_do_parse(mptest_sym_build* build_in, const mptest__str_view in, const char** err_msg, mptest_size* err_pos) {
...
MN_INTERNAL int mptest__sym_equals(mptest_sym* sym, mptest_sym* other, mptest_int32 sym_ref, mptest_int32 other_ref) {
...
MN_INTERNAL int mptest__str_grow(mptest__str* str, mptest_size new_size) {
