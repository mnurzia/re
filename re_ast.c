...
MN_INTERNAL void re__ast_destroy(re__ast* ast) { MN__UNUSED(ast); }
...
re__ast_root_new(re__ast_root* ast_root, re__ast ast, mn_int32* out_ref)
{
...
MN_INTERNAL re_error re__ast_root_add_child(
    re__ast_root* ast_root, mn_int32 parent_ref, re__ast ast, mn_int32* out_ref)
{
  re_error err = RE_ERROR_NONE;
...
/*
MN_INTERNAL re_error re__ast_root_add_sibling(re__ast_root* ast_root, mn_int32
prev_sibling_ref, re__ast ast, mn_int32* out_ref) { re_error err =
...
MN_INTERNAL re_error re__ast_root_add_charclass(
    re__ast_root* ast_root, re__charclass charclass, mn_int32* out_ref)
{
  return re__charclass_arena_add(&ast_root->charclasses, charclass, out_ref);
}

MN_INTERNAL const re__charclass*
re__ast_root_get_charclass(const re__ast_root* ast_root, mn_int32 charclass_ref)
{
  return re__charclass_arena_getcref(&ast_root->charclasses, charclass_ref);
}

MN_INTERNAL re_error
re__ast_root_add_str(re__ast_root* ast_root, mn__str str, mn_int32* out_ref)
{
...
{
  mn__str new_str;
  re_error err = mn__str_init_n(
      &new_str, mn__str_view_get_data(&group_name),
...
re__ast_root_get_group(re__ast_root* ast_root, mn_uint32 group_number)
{
  mn__str_view view;
  mn__str_view_init(
      &view, mn__str_vec_getref(&ast_root->group_names, (mn_size)group_number));
  return view;
}
